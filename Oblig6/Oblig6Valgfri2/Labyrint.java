...
public class Labyrint{
    ...
    public void leggTilVei(ArrayList<Tuppel> al){
        utveier.add(al);
    }
    public ArrayList<ArrayList<Tuppel>> finnUtvei(int rad, int kolonne){
        if(rad >= rader){
        ...
        if(kolonne >= kolonner){
            ...
            ArrayList<Tuppel> veien = new ArrayList<>();
    ...
    private void resetBesokte(){
        for(int i = 0; i < rader; i++){
            for(int y = 0; y < kolonner; y++){
                if(lab[i][y].besoekt == true){
                    lab[i][y].besoekt = false;
                }
            }
        }
                ...
                for(String tegn : tegnpaaLinje){
                    ...
                    if(tegn.equals("#")){
                        ...
                        if(rad == 0 || rad == rader -1 || kol == 0 || kol == kolonner -1){//sjekker om vi er i utkanten av labyrinten -> da er dette en åpning som må legges inn
                            tmp = new Aapning(rad, kol, this);
                            lab[rad][kol] = tmp;
                        } else{
                            tmp = new HvitRute(rad, kol, this);
                            lab[rad][kol] = tmp;
                        }
                    } else{
